...
/**
 * FIX: comparação de project_id agora normaliza ambos os lados para string
 * antes de comparar, e usa igualdade estrita só quando os dois valores
 * existem. Antes, quando s.project_id vinha undefined/null de cenários
 * antigos sem vínculo, o filtro deixava passar mesmo sem bater com o
 * projeto selecionado — por isso "só funcionava selecionando 1 a 1".
 */
function matchesProject(itemProjectId: string | undefined | null, selectedPlanId: string): boolean {
  if (!selectedPlanId) return true               // nenhum projeto selecionado = mostra tudo
  if (!itemProjectId) return false                 // projeto selecionado mas item não tem vínculo = não mostra
  return String(itemProjectId) === String(selectedPlanId)
}
...
export const selectFilteredScenarios = createSelector(
  ...
  (scenarios, planId, search) => {
    ...
    return scenarios.filter(s => {
      if (!matchesProject(s.project_id, planId)) return false
...
export const selectFilteredIssues = createSelector(
  ...
  (issues, planId, search) => {
    ...
    return issues.filter(i => {
      if (!matchesProject(i.project_id, planId)) return false
...
export const selectDashboardMetrics = createSelector(
  ...
  (allScenarios, allIssues, areas, planId) => {
    const scenarios = allScenarios.filter(s => matchesProject(s.project_id, planId))
    const issues     = allIssues.filter(i => matchesProject(i.project_id, planId))
    ...
    // "concluido" removido por regra de negócio — não existe mais como status de cenário
    ...
    return {
      total, todo, em_teste, bloqueado, falha, sucesso, executed,
