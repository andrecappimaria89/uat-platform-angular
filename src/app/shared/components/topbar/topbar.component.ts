...
import { setGlobalSearch, selectPlan } from '../../../core/services/store.actions'
...
@Component({
  ...
  template: `
    <header class="topbar">
      ...
      <div class="topbar-right">
        <!-- ITEM 3/6: Default "Todos os Projetos" — value='' -->
        ...
        <span *ngIf="(isLocked$ | async)" class="lock-badge">
          <mat-icon>lock</mat-icon> Concluído
        </span>
  ...
  `,
  styles: [`
    .topbar { display:flex; align-items:center; gap:12px; padding:10px 24px; background:white; border-bottom:0.5px solid rgba(0,0,0,.08); min-height:52px; flex-shrink:0; }
    .title-wrap { display:flex; align-items:center; gap:4px; cursor:default; flex-shrink:0; }
    .topbar-title { font-size:15px; font-weight:500; white-space:nowrap; margin:0; }
    .info-icon { font-size:14px; width:14px; height:14px; color:#888780; opacity:.5; cursor:help; }
    .info-icon:hover { opacity:1; color:#185FA5; }
    .topbar-center { flex:1; max-width:320px; }
    .search-wrap { display:flex; align-items:center; gap:6px; background:#f5f5f5; border-radius:8px; padding:6px 10px; }
    ...
    .project-select { font-size:13px; border:0.5px solid rgba(0,0,0,.12); border-radius:8px; padding:6px 10px; background:white; max-width:200px; }
    .lock-badge { display:flex; align-items:center; gap:4px; background:#FAEEDA; color:#854F0B; font-size:12px; font-weight:500; padding:4px 10px; border-radius:8px; }
...
export class TopbarComponent {
  ...
  onPlanChange(e: any) { this.store.dispatch(selectPlan({ id: (e.target as HTMLSelectElement).value })) }
}
