...
export class AreasComponent implements OnInit {
  ...
  save() {
    ...
    if (this.editingArea) {
      this.store.dispatch(updateArea({ area: { ...this.editingArea, ...this.form, members } as any }))
      this.snack.open('Área atualizada.','OK',{duration:3000})
    } else {
      this.store.dispatch(addArea({ area: { id:crypto.randomUUID(), ...this.form, members, created_at:new Date().toISOString() } as any }))
