...
import { selectIssues, selectSelectedPlanId } from '../../core/services/store.selectors'
...
@Component({
  ...
  template: `
    <div class="page">
      ...
      <div class="content">
        ...
        <!-- Tabela por severidade — dados centralizados -->
        <div class="card">
          ...
          <table class="sev-table">
            ...
            <tbody>
              ...
              <tr *ngIf="sevStats.length === 0">
                <td colspan="6" class="empty">Nenhuma issue registrada{{ planFilterLabel }}.</td>
              </tr>
            </tbody>
          </table>
        </div>
  ...
  styles: [`
    ...
    .stat-card { background:white; border:0.5px solid rgba(0,0,0,.08); border-radius:12px; padding:16px; border-left:4px solid transparent; text-align:center; }
    ...
    .sev-table th { text-align:center; padding:8px 12px; font-size:11px; font-weight:600; color:#888780; text-transform:uppercase; background:#f8f9fa; border-bottom:0.5px solid rgba(0,0,0,.08); }
    .sev-table th:first-child { text-align:left; }
    .sev-table td { padding:8px 12px; border-bottom:0.5px solid rgba(0,0,0,.05); text-align:center; }
    .sev-table td:first-child { text-align:left; }
    .num { font-weight:500; }
...
export class SumarioComponent implements OnInit, OnDestroy {
  private destroy$ = new Subject<void>()

  critical = 0; overdue = 0; open = 0; closed = 0
  sevStats: any[] = []
  planFilterLabel = ''
  ...
  ngOnInit() {
    // FIX: agora reage a selectSelectedPlanId — antes mostrava sempre
    // todas as issues, ignorando o projeto selecionado no topbar
    combineLatest([
      this.store.select(selectIssues),
      this.store.select(selectSelectedPlanId),
    ]).pipe(takeUntil(this.destroy$)).subscribe(([allIssues, planId]) => {
      const issues = planId
        ? allIssues.filter(i => i.project_id === planId)
        : allIssues
      this.planFilterLabel = planId ? ' para este projeto' : ''
