    ...
    <div class="page">
      ...
      <div class="content">
        ...
        <div class="filters-bar">
          ...
          <button class="btn-clear" (click)="clearFilters()"><mat-icon>clear</mat-icon> Limpar Filtros</button>
        ...
        <div *ngIf="showForm" class="card form-card">
          ...
          <div class="form-grid">
            ...
            <!-- ITEM 4: Responsável — preenchido automaticamente pelos usuários da área -->
            <div class="form-field">
              <label>
                Responsável
                <span class="required-hint" *ngIf="form.area_name && areaUsers.length > 0">
                  (usuários da área selecionada)
                </span>
                <span class="required-hint" *ngIf="form.area_name && areaUsers.length === 0">
                  (nenhum usuário nesta área — edite manualmente)
                </span>
              </label>
              ...
              <tbody>
                <tr *ngFor="let i of paged">
                  ...
                  <td class="actions-col">
                    <button class="icon-btn" (click)="openEdit(i)" matTooltip="Editar" matTooltipPosition="above"><mat-icon>edit</mat-icon></button>
                    <button class="icon-btn danger" *ngIf="!locked" (click)="remove(i)" matTooltip="Excluir" matTooltipPosition="above"><mat-icon>delete</mat-icon></button>
                  </td>
                </tr>
              </tbody>
            </table>
...
export class IssuesComponent implements OnInit, OnDestroy {
  ...
  allIssues:Issue[]=[]; filtered:Issue[]=[]; paged:Issue[]=[]; areas:any[]=[]; allUsers:any[]=[]; allScenarios:Scenario[]=[]; openPlans:TestPlan[]=[]
  filteredScenariosByProject: Scenario[] = []
  areaUsers: any[] = []  // ITEM 4: usuários da área selecionada
  ...
  // ITEM 4: ao selecionar área, busca usuários vinculados e preenche o campo responsável
  onAreaChange(areaName: string) {
    this.form.area_name = areaName
    const area = this.areas.find(a => a.name === areaName) as any
    const members: string[] = area?.members ?? []

    // Busca os objetos User correspondentes aos membros da área
    this.areaUsers = this.allUsers.filter(u => members.includes(u.name))

    if (this.areaUsers.length === 1) {
      // Se só tem 1 usuário, preenche automaticamente
      this.form.responsible_name = this.areaUsers[0].name
    } else if (this.areaUsers.length > 1) {
      // Se tem mais de 1, sugere o responsável principal da área mas permite troca
      const responsible = area?.responsible_name
      this.form.responsible_name = responsible && this.areaUsers.some(u => u.name === responsible)
        ? responsible
        : ''
    } else {
      // Sem usuários vinculados — campo livre para edição manual
  ...
  openNew() {
    this.editingIssue=null; this.formSubmitted=false; this.filteredScenariosByProject=[]; this.areaUsers=[]
  ...
  openEdit(i:Issue) {
    ...
    // Carregar usuários da área já selecionada
    if (i.area_name) this.onAreaChange(i.area_name)
    this.showForm=true
    setTimeout(()=>document.querySelector('.form-card')?.scrollIntoView({behavior:'smooth'}),100)
  }
