...
@Component({
  selector: 'app-plan',
  standalone: true,
  imports: [CommonModule, FormsModule, MatButtonModule, MatIconModule, MatSnackBarModule, MatDialogModule, TopbarComponent],
  template: `
    <div class="page">
      <app-topbar title="Plano de Testes">
        <button mat-stroked-button (click)="showNewForm = !showNewForm; editing = false">
          <mat-icon>add</mat-icon> Novo Plano
        </button>
        <ng-container *ngIf="plan$ | async as p">
          <button mat-stroked-button *ngIf="!(locked$ | async) && !editing && p?.id" (click)="startEdit(p)">
            <mat-icon>edit</mat-icon> Editar
          </button>
          <button mat-flat-button color="warn" *ngIf="!(locked$ | async) && !editing && p?.id" (click)="conclude(p)">
            <mat-icon>check_circle</mat-icon> Concluir
          </button>
        </ng-container>
        <button mat-flat-button color="primary" *ngIf="editing" (click)="save()">
          <mat-icon>save</mat-icon> Salvar
        </button>
        <button mat-stroked-button *ngIf="editing" (click)="cancelEdit()">
      ...
      <div class="content">
        ...
        <div *ngIf="showNewForm" class="card new-plan-card">
          ...
          <div class="form-actions">
            <button mat-stroked-button (click)="showNewForm = false">Cancelar</button>
          ...
          <table class="plans-table">
            ...
            <tbody>
              <tr *ngFor="let p of plans$ | async" [class.selected-row]="p.id === (selectedPlanId$ | async)">
                ...
                <td>
                  <button mat-stroked-button (click)="selectThisPlan(p.id)">
